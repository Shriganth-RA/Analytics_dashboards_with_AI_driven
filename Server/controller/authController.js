...
export const register = async (req, res) => {
    try {
        ...
        const newUser = new userModel({ name, email, password: hashedPassword, role, profilePic });
        await newUser.save();
...
export const login = async (req, res) => {
    try {
        ...
        return res.status(201).json({ success: true, message: "Login successfully.", userData: { id: user._id, name: user.name, email: user.email, role: user.role, token: token } })
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message })
    }
}
